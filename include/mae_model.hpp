...
torch::Tensor get_2d_sincos_pos_embed(int64_t embed_dim, int64_t grid_size, bool cls_token = false);
...
public:
    PatchEmbedImpl(int64_t img_size = 224, int64_t patch_size = 16, int64_t in_chans = 3, int64_t embed_dim = 768);
...
public:
    MlpImpl(int64_t in_features, int64_t hidden_features, int64_t out_features, float drop = 0.);
...
public:
    AttentionImpl(int64_t dim, int64_t num_heads = 8, bool qkv_bias = false, float attn_drop = 0., float proj_drop = 0.);
    torch::Tensor forward(torch::Tensor x);
...
public:
    BlockImpl(int64_t dim, int64_t num_heads, double mlp_ratio = 4., bool qkv_bias = false, 
              float drop = 0., float attn_drop = 0., float drop_path = 0.);
    torch::Tensor forward(torch::Tensor x);
